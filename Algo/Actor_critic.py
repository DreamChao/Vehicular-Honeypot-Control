...
# the total episodes of training  总训练次数
EPISODES = 1000
# the total steps in one episode  每个回合的步数
STEPS = 300
# used to caculate the discount reward
GAMMA = 0.9
# the update times of actor and critic 更新次数
...
class PPO():
    ...
    def build_actor_net(self, name, trainable):
        with tf.variable_scope(name):
            l1 = tf.layers.dense(self.state_input, 100, tf.nn.relu, trainable=trainable)
            l2 = tf.layers.dense(l1, 50, tf.nn.relu, trainable=trainable)
            logits = tf.layers.dense(l2, self.action_dim, trainable=trainable)
    ...
    def Choose_Action(self, s):
        s = np.array(s)[np.newaxis, :]
        action_dist = self.sess.run(self.sample_from_oldpi, {self.state_input: s})[0]
        action = np.argmax(action_dist)  # 选择得分最高的动作
        return action
    ...
    def get_v(self, s):
        if s.ndim < 2: np.array(s)[np.newaxis, :]
